...
public class JdbcOrderRepository implements  OrderRepository{
    ...
    private void saveIngredientRef(long tacoId, List<Ingredient> ingredientRefs){

        int key = 0;
        for(Ingredient ingredient : ingredientRefs){
            jdbcOperations.update(
                    "insert int Ingredient_Ref (ingredient, taco, taco_key)" +
                            "values(?,?,?)", ingredient, tacoId, key++);
        }
    }
}
